    ...
    if (isset($_SESSION["user"])) {
        ...
        if (empty($activityName) || empty($organizingUnit) || empty($schemeName) || empty($yearOfActivity) || empty($teachersParticipated) || empty($studentsParticipated) || empty($file_name)) {
            ...
            if (move_uploaded_file($_FILES['file']['tmp_name'], $file_destination)) {
                $sql = "INSERT INTO 92eleven (username, activityName, organizingUnit, schemeName, yearOfActivity, teachersParticipated, studentsParticipated, file_name) 
                    VALUES ('$username', '$activityName', '$organizingUnit', '$schemeName', '$yearOfActivity', '$teachersParticipated', '$studentsParticipated', '$file_name')";
