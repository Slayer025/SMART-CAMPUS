...
// Fetch data from the database for the "93twelve" table
$sql = "SELECT * FROM 93twelve";
...
<body>
        ...
        <div class="rep-container">
            ...
            <div class="filter-container">
                ...
                <select class="filter-button" id="deptFilter">
                    <option class="button" value="">Partnering Institutions</option>
                    <?php
                    // Generate department options
                    $deptResult = mysqli_query($conn, "SELECT DISTINCT partneringInstitution FROM 93twelve");
                    while ($deptRow = mysqli_fetch_assoc($deptResult)) {
                        echo "<option value='{$deptRow['partneringInstitution']}'>" . htmlspecialchars($deptRow['partneringInstitution']) . "</option>";
                    }
                    ?>
                </select>
            </div>
            ...
            <table class="table">
                <thead>
                    <tr>
                        <th>Sl. No.</th>
                        <th>Title of the Linkage</th>
                        <th>Partnering Institution</th>
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr>";
                        echo "<td>{$row['slNo']}</td>";
                        echo "<td>{$row['linkageTitle']}</td>";
                        echo "<td>{$row['partneringInstitution']}</td>";
                        echo "<td>{$row['yearOfCommencement']}</td>";
                        echo "<td>{$row['durationFrom']}</td>";
                        echo "<td>{$row['durationTo']}</td>";
                        echo "<td>{$row['natureOfLinkage']}</td>";
                        echo "</tr>";
                    }
                    ?>
                </tbody>
    ...
    <script>
        ...
        function exportToExcel() {
            ...
            /* Save to file */
            XLSX.writeFile(wb, '3.5.1.report.xlsx');
        }
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            for ($i = $currentYear; $i > $currentYear - 5; $i--) {
                ...
                $sql = "SELECT COUNT(*) as count FROM 93twelve WHERE yearOfCommencement = '$i'";
